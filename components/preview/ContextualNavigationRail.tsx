 ...
 * Status: ACTIVE
 *
 * One global Quick entry point for deeper contextual levels.
 * The Universe root uses its own Quick surface because its experience
 * controls are owned by UniverseStage.
 */

import { useEffect, useMemo, useRef, useState } from "react";
import { ArrowLeft, ArrowRight, Check, Compass, Home, Layers, SlidersHorizontal, X } from "lucide-react";
...
export const ContextualNavigationRail = ({
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: MouseEvent | TouchEvent) => {
      ...
      if (target && panelRef.current && !panelRef.current.contains(target)) setIsExpanded(false);
    };
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") { setIsExpanded(false); triggerRef.current?.focus(); }
    };
  ...
  if (activeLevel === "L1") return null;

  return (
    <nav aria-label="AAi Quick navigation" className={`fixed left-2 sm:left-4 top-1/2 -translate-y-1/2 z-[70] select-none ${className}`}>
      {!isExpanded ? (
        <button
          ref={triggerRef}
          type="button"
          ...
          className={`group flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-2xl border shadow-2xl backdrop-blur-xl transition-all duration-200 ${isDark ? "border-cyan-400/30 bg-[#020d1c]/90 text-cyan-300 hover:border-cyan-300/70 hover:bg-[#041a33]" : "border-slate-300 bg-white/95 text-slate-600 hover:border-cyan-400"}`}
        >
