...
export const DomainContextBanner: React.FC<DomainContextBannerProps> = ({
  ...
  const hasSubdomain =
    Boolean(subdomain) && !hasCapability && !hasBundle && !hasSolution;
  const hasDomain =
    Boolean(domain) &&
    !hasSubdomain &&
    !hasCapability &&
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (
        subdomainContainerRef.current &&
        !subdomainContainerRef.current.contains(e.target as Node)
      ) {
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 min-h-[36px]">
        ...
        <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 text-xs font-mono min-w-0 flex-1">
          ...
          >
            <Compass
              className={`w-3.5 h-3.5 ${isRoot ? (isDark ? "text-[#00e3fd]" : "text-indigo-600") : isDark ? "text-[#9ec5de]" : "text-slate-500"}`}
            />
            <span className="truncate font-semibold">
              M01 Solution Universe
            </span>
          </button>
          ...
          {domain ? (
            <>
              <span
                className={`font-mono text-sm font-bold select-none px-0.5 ${isDark ? "text-[#00e3fd]/50" : "text-slate-400"}`}
              >
                ›
              </span>
              <button
                  ...
                  >
                    <div className="flex items-center justify-between px-2.5 py-1.5 mb-1.5 border-b border-inherit">
                      <span className="font-mono text-[11px] font-bold uppercase tracking-wider text-[#00e3fd]">
                        Sub-Worlds in {domain?.id || "Domain"} (
                        {subdomains.length})
                      </span>
                ...
                >
                  ...
                  <div className="max-h-72 overflow-y-auto space-y-0.5 pr-1 scrollbar-thin">
                    {allDomains.map((dm) => {
                      ...
                      const dmColor = dm.color || dm.accentColor || "#00e3fd";
                      return (
                        <button
                          key={dm.id}
                          type="button"
