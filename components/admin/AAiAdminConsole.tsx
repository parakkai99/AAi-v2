 ...
 * Version: 1.1.0
 *
 * Framework administration only. Customer content, customer databases and
 * private signing material are never displayed in the client console.
 */
      ...
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <nav className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2" aria-label="AAi administration sections">
          {sections.map((section, index) => (
            <div key={section} className={`rounded-xl border p-3 text-xs font-mono ${index === 0 || index === 2 || index === 3 ? 'border-[#00dfff]/30 bg-[#031c33]/80 text-[#00e3fd]' : 'border-white/10 bg-white/[0.02] text-[#82a5bb]'}`}>
              {section}
            </div>
          ))}
        </nav>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <section className="xl:col-span-2 rounded-2xl border border-[#00dfff]/20 bg-[#061525]/80 p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="ml-auto text-[10px] font-mono uppercase px-2 py-1 rounded-full border border-emerald-400/30 bg-emerald-400/10 text-emerald-300">{license.status}</span>
            </div>
...
function Metric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-xl border border-[#00dfff]/10 bg-[#020914]/60 p-3 text-center"><div className="text-lg font-bold text-[#00dfff]">{value}</div><div className="text-[9px] uppercase tracking-wider text-[#5f8299] font-mono">{label}</div></div>;
}
