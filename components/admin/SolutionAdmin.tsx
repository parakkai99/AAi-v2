...
import { ArrowLeft, Bot, CheckCircle2, FileJson, Image, LayoutTemplate, Palette, Save, Send, Settings2, Sparkles, Upload, Workflow, X } from 'lucide-react';
...
export const SolutionAdmin: React.FC<SolutionAdminProps> = ({ solutionId, onBackToAAiAdmin }) => {
  ...
  const update = (patch: Partial<SolutionAdminConfig>) => setConfig((current) => ({ ...current, ...patch, publishState: current.publishState === 'published' ? 'draft' : current.publishState }));
      ...
      <div className="max-w-[1700px] mx-auto space-y-5">
        <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-3 mt-1"><h1 className="text-3xl sm:text-4xl font-bold">{name} Admin</h1><span className="rounded-lg border border-cyan-400/20 px-2 py-1 text-[10px] font-mono text-cyan-300">{solutionId}</span></div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={onBackToAAiAdmin ?? (() => window.history.pushState({}, '', '/admin'))} className="inline-flex items-center gap-2 rounded-xl border border-cyan-400/30 bg-[#031c33] px-3 py-2 text-xs font-mono text-cyan-300"><ArrowLeft className="w-4 h-4"/> AAi-Admin</button>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-5">
          <aside className="rounded-2xl border border-white/10 bg-[#061525]/80 p-3 h-fit lg:sticky lg:top-4">
            ...
            <div className="space-y-1">
              {sections.map(([label, Icon]) => <button key={label} type="button" onClick={() => setActiveSection(label)} className={`w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs text-left ${activeSection === label ? 'bg-cyan-400 text-[#00131f] font-bold' : 'text-[#9bb6c7] hover:bg-white/5'}`}><Icon className="w-4 h-4"/>{label}</button>)}
            </div>
          </aside>
          ...
          <main className="space-y-5">
            ...
            {activeSection === 'Publish' && <PublishPanel config={config} onSave={save} onPublish={publish}/>} 
...
function PublishPanel({ config, onSave, onPublish }: { config: SolutionAdminConfig; onSave: () => void; onPublish: () => void }) { return <Panel title="Publish & Lifecycle" icon={Send}><div className="grid grid-cols-2 md:grid-cols-4 gap-3"><Metric label="State" value={config.publishState}/><Metric label="Version" value={`v${config.version}`}/><Metric label="Theme" value={config.themeId}/><Metric label="Layout" value={config.layoutId}/></div><div className="flex gap-2 mt-5"><button type="button" onClick={onSave} className="inline-flex items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-xs"><Save className="w-4 h-4"/> Save Draft</button><button type="button" onClick={onPublish} className="inline-flex items-center gap-2 rounded-lg bg-emerald-400 px-3 py-2 text-xs font-bold text-[#001a10]"><Send className="w-4 h-4"/> Publish</button></div></Panel>; }
