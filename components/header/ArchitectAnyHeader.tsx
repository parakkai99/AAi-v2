...
export const ArchitectAnyHeader: React.FC<ArchitectAnyHeaderProps> = ({
  ...
  useEffect(() => {
    const handleAppStackToggle = () => {
      window.dispatchEvent(new CustomEvent('aai:close-json-inspector'));
      setIsAppStackOpen((open) => !open);
    };
    const handleInspectorOpen = () => setIsAppStackOpen(false);
    window.addEventListener('aai:toggle-app-stack', handleAppStackToggle);
    window.addEventListener('aai:open-json-inspector', handleInspectorOpen);
    return () => {
      window.removeEventListener('aai:toggle-app-stack', handleAppStackToggle);
      window.removeEventListener('aai:open-json-inspector', handleInspectorOpen);
    };
  }, []);
      ...
      <div className="flex items-center px-3 sm:px-5 lg:px-8 h-full max-w-[1600px] mx-auto gap-2 sm:gap-4">
        ...
        <div className="relative z-[110]">
          <button
            ...
            className={`relative w-9 h-9 rounded-xl transition-all cursor-pointer flex items-center justify-center touch-manipulation ${isDark
          ...
          {isAppStackOpen && (
            <div
              className={`absolute left-0 mt-2 w-[min(18rem,calc(100vw-1rem))] rounded-2xl border shadow-2xl p-3 backdrop-blur-xl z-[120] ${isDark
...
function StackButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      role="menuitem"
      className={`rounded-xl border px-3 py-3 text-left transition-all touch-manipulation ${
        active
          ? isDark
